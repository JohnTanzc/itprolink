...
@section('content')
    ...
    <div class="dashboard-content-wrap">
        ...
        <h3 class="fs-22 font-weight-semi-bold mb-3 ms-3">Tutee</h3>
        ...
        <div class="card mx-3">
            <div class="card-body">
                ...
                <table class="table table-hover align-middle">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th scope="col" style="width: 5%;">Course Enrolled</th>
                            {{-- <th scope="col" style="width: 5%;">Verification Documents</th> --}}
                            <th scope="col" style="width: 5%;">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($tutees as $index => $tutee)
                            <tr>
                                <th scope="row">{{ $index + 1 }}</th>
                                <td>{{ $tutee->fname }} {{ $tutee->lname }}</td>
                                <td>{{ $tutee->role }}</td>
                                <td>{{ $tutee->email }}</td>
                                <td>{{ $tutee->active ? 'Active' : 'Not Active' }}</td>
                                <td>{{ $tutee->verified ? 'Verified' : 'Not Verified' }}</td>
                                <td>{{ $tutee->enrollments->count() }}</td> <!-- Adjust relationship name if needed -->
                                {{-- <td>{{ $tutee->verifications }}</td> <!-- Adjust field name if needed --> --}}
                                <td>
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="text-center">No tutee found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                <div class="d-flex justify-content-center">
                    {{ $tutees->links() }} <!-- This will generate the pagination links -->
                </div>
